...
/**Directly copied from CCBoardState, except the turn_number is incremented every turn regardless of player
 * then the canCapture method checks if turn number > 2 to decide if it can capture any stones.
 * The max turn was also doubled to account for the higher turn number.
 * This now fixes the problem of player two reading incorrect states in the alphabeta/minimax algorithm, which before
 * essentially caused player 2 to do a random move and let player 1 run all him/her because of that.
 */
...
public class MyBoardState extends CCBoardState{
    ...
    public final static int MAX_TURN = 10000;
    ...
    public MyBoardState(int[][] board, int turn_number, int winner, int turn_player, int plays_first){
        super();
    ...
    public void setTurnsPlayed(int actualTurns){
    	turn_number = actualTurns;
    }
    ...
    public boolean isLegal(CCMove m){
        ...
        return m.getMoveType() == MoveType.PIT && board[turn_player][m.getPit()] > 1;
    }
    ...
    @Override
    public Object clone() {
        return new MyBoardState(board, turn_number, winner, turn_player, plays_first);
    }
